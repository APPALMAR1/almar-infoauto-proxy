...
const TIMEOUT_MS       = 6000; // margen bajo el maxDuration:10 del proxy
...
export async function getValidToken() {
  ...
  const { data } = await withTimeout(
    sb.from(TOKEN_TABLE).select('*').eq('id', 1).single(),
    TIMEOUT_MS,
    'supabase-get-token'
  );
...
async function saveToken(json) {
  ...
  await withTimeout(sb.from(TOKEN_TABLE).upsert(record), TIMEOUT_MS, 'supabase-save-token');
}
